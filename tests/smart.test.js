...

{
  ...
  assert.deepEqual(
    ...
    ["智能选择", "香港自动", "新加坡自动", "日本自动", "美国自动", "欧洲自动", "亚洲其他自动", "DIRECT"],
    "默认代理 should become a smaller service-first entry"
  );
}

{
  ...
  assert.deepEqual(
    ...
    ["新加坡自动", "日本自动", "美国自动", "香港自动", "韩国自动", "台湾自动", "欧洲自动", "亚洲其他自动", "智能选择"],
    "AIGC should expose a single shared AI candidate order"
  );

  assert.deepEqual(
    getGroup(result, "OpenAI").proxies,
    ["AIGC", "新加坡自动", "日本自动", "美国自动", "欧洲自动", "智能选择"],
    "OpenAI should be a thin wrapper over AIGC"
  );

  assert.deepEqual(
    getGroup(result, "Copilot").proxies,
    ["AIGC", "DIRECT", "美国自动", "日本自动", "新加坡自动", "智能选择"],
    "Copilot should keep DIRECT higher than other AI services"
  );

  assert.deepEqual(
    getGroup(result, "GitHub").proxies,
    ["智能选择", "美国自动", "日本自动", "新加坡自动", "欧洲自动", "DIRECT"],
    "GitHub should use a workflow-oriented candidate order"
  );

  assert.deepEqual(
    getGroup(result, "微软服务").proxies,
    ["DIRECT", "智能选择", "美国自动", "日本自动", "新加坡自动", "欧洲自动"],
    "Microsoft should remain direct-first"
  );
}
